...
class TimelineModel:
    ...
    def get_available_angles(self, frame_idx: int) -> List[int]:
        """Restituisce la lista degli angoli disponibili per un frame."""
        if 0 <= frame_idx < len(self.frames):
            return sorted([a.angle for a in self.frames[frame_idx].angles])
        return []
    ...
    def set_duration(self, index, ms):
        if 0 <= index < len(self.frames):
            ms = max(1, int(ms))
            self.frames[index].duration_ms = ms
            for angle_data in self.frames[index].angles:
                angle_data.duration_ms = ms
            self._calc_total()
